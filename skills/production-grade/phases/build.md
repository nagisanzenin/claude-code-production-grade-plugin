...
2. `Claude-Production-Grade-Suite/.orchestrator/oracle-full.sh` — the **full oracle**: test suite + build + **deep boot smoke**. The smoke MUST exercise real endpoints/flows (create then read a resource; hit an authed route), **not just `/health`** — a health-only smoke passes an app whose every feature 500s (this exact failure was observed in the v5.5 shakedown: unit tests green, `/health` green, every real endpoint broken). **Include at least one adversarial/edge input** in the smoke (an unusual/unsafe value — a URL with spaces or `<>`, a boundary number, a malformed field), not only happy-path values: a smoke that reuses the unit tests' inputs inherits their blind spots (in the v5.5.1 dogfood a "verbatim redirect" bug hid behind safe-char-only smoke AND tests). Run at loop exits, wave merges, and phase transitions — not per edit.
